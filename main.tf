...
locals {
  ...
  # Naming convention: {env}-roc-terraform-{project}-{resource}-{suffix}
  name_prefix  = "${var.environment}-roc-terraform-${var.project}"
  short_prefix = "${var.environment}-roc-${substr(var.project, 0, 2)}"
